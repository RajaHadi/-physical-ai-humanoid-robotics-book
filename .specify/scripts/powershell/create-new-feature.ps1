...
param(
    ...
    [string]$FeatureDescriptionFile # New parameter for reading from file
)
...
if ($Help) {
    ...
    Write-Host "  ./create-new-feature.ps1 -FeatureDescriptionFile './my_feature_desc.txt' -ShortName 'file-based-feature'"
    exit 0
}
...
if (-not [string]::IsNullOrEmpty($FeatureDescriptionFile)) {
    if (Test-Path $FeatureDescriptionFile -PathType Leaf) {
